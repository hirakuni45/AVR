...
namespace app {
	...
	*/
	...
	void menu::init()
	{
		...
		icon_ = task_.eeprom_.read(device::eeprom::slot::menu_pos);
		if(icon_ > 3 || icon_ < 0) icon_ = 0;		
	}
	...
	*/
	...
	void menu::service()
	{
		if(goto_) {
			...
			if(goto_ == 0) {
				if(icon_ == 0 || icon_ == 1) {
					if(icon_ == 0) task_.set_share(0);
					else task_.set_share(kt_min_);
					task_.start<ktimer>();
				} else if(icon_ == 2) task_.start<tetris>();
				else if(icon_ == 3) task_.start<timer>();
			}
			if((goto_ % 12) < 5) {
				uint8_t no = icon_;
				if(icon_ >= 1) --no;
		...
		if(swi.last_touch(60 * 3)) {
			task_.eeprom_.write(device::eeprom::slot::menu_pos, icon_);
		}

		if(icon_ != icon__) {
			if(icon_ < icon__) speed_ = 80000;
			else if(icon_ > icon__) speed_ =-80000; 
		}
		icon__ = icon_;
